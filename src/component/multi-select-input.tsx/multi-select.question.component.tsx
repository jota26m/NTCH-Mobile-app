...
export class MultiSelectQuestion extends React.Component<Props, State> {
    constructor(props: Props) {
        super(props);
        let selectedValues: any = [];
        this.props.selectedValues.forEach((value: any) => {
            let opt = this.props.options.find((opt: any) => opt.id == value);
            if (opt != undefined) selectedValues.push(opt.id);
        });
        this.state = {
            selectedValues
        };
    ...
    setValue = (value: number) => {
        ...
        if (selectedValues.includes(value)) {
            selectedValues = selectedValues.filter((val: number) => val != value);
        } else {
